...
RTC_DATA_ATTR int sleepDuration = 300; // Seconds (default 600s = 10 min)
RTC_DATA_ATTR int bootCount = 0;

// Hardcoded Credentials
const char* SSID1 = ""; // CHANGE ME
const char* PASSWORD1 = ""; // CHANGE ME
const char* SSID2 = ""; // CHANGE ME
const char* PASSWORD2 = ""; // CHANGE ME
const char* CONFIG_URL1 = "http://<SERVER-IP>:8000/api/config"; // UPDATE WITH SERVER IP
const char* CONFIG_URL2 = "http://<SERVER-IP>:8000/api/config"; // UPDATE WITH SERVER IP
const char* SERVER_URL1 = "http://<SERVER-IP>:8000/api/sensor"; // UPDATE WITH SERVER IP
const char* SERVER_URL2 = "http://<SERVER-IP>:8000/api/sensor"; // UPDATE WITH SERVER IP
...
void initSensors() {
  ...
  if (!bmp.begin()) {
    Serial.print(F("Pressure sensor init failed.\n"));
    return;
  }
  ...
  if (!veml.begin()); {
    Serial.print(F("Light sensor init failed.\n"));
    return;
    delay(10);
  }
  Serial.print(F("All sensors initialized!")); 
...
void sensorTask(void *pvParameters) {
  ...
  Serial.printf("[INFO] Temp(F): %.2f, Humidity: %.2f, Lux: %.2f, Moisture: %.2f, Pressure(Pa): %.2f\n", tempF, humidity, lux, moisture, pressure);
